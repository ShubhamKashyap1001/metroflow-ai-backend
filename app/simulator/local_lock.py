"""Same-host, cross-process file lock - the Phase 7A fallback for
`app/simulator/leader_election.py` when Redis coordination is
unavailable (disabled, or never reachable since process startup).

Background / the bug this closes
---------------------------------
Phase 4 (see docs/background-jobs-and-leader-election.md) made `LeaderElection` coordinate
across worker processes using a Redis lease - but it explicitly
documented one remaining gap: "if you run MULTIPLE workers with Redis
unreachable for the process's entire lifetime, you will get duplicate
loops, because there is no way to coordinate N independent processes
without a shared coordinator." That gap was real: with no Redis, every
process reached the same `if not self.is_active(): start it` branch
and each one independently concluded "I must be alone" - the exact
mechanism that produces duplicate simulators under
`uvicorn --workers N` (or several `uvicorn`/`gunicorn` processes
started by hand) whenever Redis isn't up yet at startup.

This module removes that gap for the common case those deployments
actually are: multiple worker processes on the *same host*, sharing
the same filesystem. `fcntl.flock()` is a kernel-enforced, exclusive,
advisory lock on an open file descriptor - the kernel itself refuses
to let a second process hold the same lock at the same time, which is
exactly the cross-process signal a plain Python/asyncio-level flag
cannot provide (an `asyncio.Lock` only ever guards concurrent tasks
*inside one process's event loop*; it is invisible to a sibling
process entirely). It is also automatically released by the OS the
moment the holding process exits or is killed - no TTL bookkeeping
needed, so a crashed leader can never leave the lock stuck held,
mirroring the crash-safety the Redis lease already had.

What this does NOT solve: processes on *different hosts* (e.g.
several container replicas, each with their own filesystem) cannot
coordinate through a local file at all. That case still requires
Redis - which is why this is a documented *fallback*, not a
replacement for the Redis path, and why it fails CLOSED rather than
assuming "I'm alone" when it can't verify exclusivity.

Same-process callers are handled correctly too: `flock()`'s
exclusivity is enforced by the kernel against every distinct *open
file description*, not just every distinct process - a second
`os.open()` + `flock(LOCK_EX | LOCK_NB)` on the same path from the
very same process (a different open file description) correctly
raises EWOULDBLOCK, verified empirically. `try_acquire`'s own
same-caller idempotency shortcut (so a `LeaderElection` polling every
few seconds isn't re-opening a file descriptor on every tick) is
scoped by an optional `holder_id` for exactly this reason - see its
docstring.
"""
import errno
import logging
