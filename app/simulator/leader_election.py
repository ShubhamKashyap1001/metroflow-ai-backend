"""Redis-lease-based leader election for background simulator loops.

Phase 4 problem this solves: when the API runs as multiple processes
(``uvicorn --workers N``, or multiple container replicas behind a load
balancer), app/main.py's lifespan() runs on EVERY process, and before
this module existed it unconditionally called start_simulator() /
start_train_tracker() / start_retention_job() on each one. Each of
those functions guarded itself with a module-level ``asyncio.Task``
variable - but that variable lives in ONE process's memory, so the
guard only ever stopped a second call *within* the same process. Every
worker independently believed it was the only one and started its own
full copy of every loop: N processes -> N x crowd_logs / train_location
writes, N x overcrowding notifications, N x WebSocket broadcasts.

The fix: instead of each process deciding for itself whether to run a
loop, all processes continuously bid for a single shared Redis lease
(see app/core/cache.py's try_acquire_or_renew_lock / release_lock).
Only the process currently holding the lease - the "leader" - actually
runs the loop; everyone else polls in standby. The lease has a TTL, so
if the leader process dies or is killed, its lease simply expires and
whichever standby process next polls picks it up - automatic failover,
no separate crash-detection mechanism needed.

One LeaderElection instance guards ONE named loop, so the crowd
simulator, train tracker, and retention job elect independently (any
one of the N processes could end up leading any subset of the three).

Redis-unavailable fallback (Phase 7A): if Redis is disabled
(CACHE_ENABLED=False / no REDIS_URL) or has never been reachable since
this process started, cross-host coordination is impossible - but
cross-PROCESS coordination on the same host still is, via a
kernel-enforced file lock (see app/simulator/local_lock.py). Every
process bidding in this state tries to acquire that local lock instead
of assuming it's alone; only the one that actually wins it runs the
loop, everyone else stays in standby. This closes the gap Phase 4 had
documented and accepted (docs/background-jobs-and-leader-election.md): "running MULTIPLE
workers with Redis unreachable for the process's entire lifetime" used
to produce duplicate loops (fail OPEN); it no longer does for
same-host deployments (`uvicorn --workers N`, a single container/VM
running several processes) - see docs/background-jobs-and-leader-election.md. The one case a
local file lock genuinely cannot help with is multiple *hosts* (e.g.
several container replicas with separate filesystems) running with
Redis unreachable for their entire lifetime; that still requires Redis
to be reachable for cross-host correctness, and is the one scenario
where this can still under-coordinate - documented, not hidden.

If Redis was reachable before and then goes down mid-flight (a real
outage, not "never configured"), this used to fail CLOSED forever:
every process stepped down and NONE of them ever ran the loop again
until Redis came back - a full, indefinite outage of the simulator
fleet triggered by a single dependency (Redis) going down, which is
the opposite of "fail-safe". Phase 7B closes that gap: once Redis has
been unreachable for at least one full lease window
(``lease_seconds``), any Redis lease that existed anywhere before the
outage is *guaranteed* to have expired by then (nobody could have
renewed it without Redis), so it's safe to treat this exactly like
"Redis never configured" and fall back to the same-host local-lock
coordinator - the simulator keeps running (same-host-coordinated) for
the rest of the outage instead of sitting fully idle. Cross-host
duplication during that fallback window is the same already-documented,
already-accepted limitation the "never reachable" case has always had
(a local file lock can't coordinate across hosts) - trading a small,
bounded, already-accepted risk for actually staying available beats
guaranteed unavailability for however long Redis stays down. Before
that lease-window grace period elapses, this still fails CLOSED
(steps down) exactly as before, so a lease genuinely still held
elsewhere can't be raced.
"""
import asyncio
import logging
import threading
...
class LeaderElection:
    """Ensures at most one process (across all API workers/replicas)
    runs `run_loop` at a time, using a Redis lease as the shared source
    of truth. `run_loop` is a zero-argument async callable that runs
    forever (like the existing simulator run_forever() functions) until
    cancelled.
    """
